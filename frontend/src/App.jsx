    ...
    <div className="w-full max-w-[1400px] flex flex-col lg:flex-row shadow-2xl overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-white/50 backdrop-blur-sm">
        ...
        <div className="h-16 bg-white/80 border-b flex items-center px-10 text-sm font-medium text-gray-500 italic">
          End-to-end encrypted communication session for candidate{" "}
          {candidateId.split("-")[0]}...
        </div>
        ...
        {role === "HR" && (
          <div className="lg:hidden px-4 py-3 bg-white border-b flex gap-3">
            ...
            {roomId && (
              <button
                onClick={createInvite}
                className="flex-1 py-3 rounded-xl border border-gray-200 bg-white text-gray-700 text-sm font-bold"
              >
                Invite
              </button>
            )}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-8 space-y-10">
              {role === "HR" && (
                <div className="text-center pb-6 border-b border-gray-50">
                  ...
                  <p className="text-sm text-gray-400">
                    Incoming referee verification data
                  </p>
                </div>
              )}
              ...
              {chat
                ...
                .map((m, i) => {
                  ...
                  const isRefereeAnswer =
                    m.sender === "REFEREE" && m.is_answer;
                  const relatedQuestion = isRefereeAnswer
                    ? getRelatedQuestion(m)
                    : null;
                      ...
                      {role === "HR" ? (
                        isRefereeAnswer ? (
                          <div className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6">
                            <p className="text-[10px] uppercase font-black text-slate-400 mb-4 tracking-tighter">
                              Verified Entry
                            </p>
                            {relatedQuestion && (
                          ...
                          <div className="max-w-[70%] px-5 py-3 rounded-2xl bg-gray-100 text-gray-700 font-medium border border-gray-200">
                            {m.text}
                          </div>
                        )
                      ) : (
                        <div
                          className={`max-w-[80%] px-6 py-4 rounded-2xl text-lg shadow-sm ${
                            isOwn
                        ...
                        >
                          {isOwn &&
                            isRefereeAnswer &&
                            relatedQuestion && (
                              <div className="mb-2 text-xs text-blue-100 border-l-2 border-blue-200 pl-3 opacity-80 italic">
                                Context: {relatedQuestion.text}
                              </div>
                            )}
                          {m.text}
                        </div>
